...
abstract class OAuthStoreMongo extends OAuthStoreAbstract
{
	/**
	...
	public function getSecretsForVerify ($consumer_key,$token,$token_type = 'access'){
		if ($token_type === false){
			$rs = $this->db->oauth_server_registry->findOne(array("osr_consumer_key" => $consumer_key, "osr_enabled" => 1));	
			if ($rs){
				$rs['token'] 		= false;
				$rs['token_secret']	= false;
				$rs['user_id']		= false;
				$rs['ost_id']		= false;
				$rs['consumer_key'] = $rs['osr_consumer_key'];
				$rs['consumer_secret'] = $rs['osr_consumer_secret'];
			}
		} else {
			$now = new Date();
			$rs = $this->db->oauth_server_registry->findOne(array("osr_consumer_key" => $consumer_key,"ost_token_type" => $token_type, "ost_token" => $token, "osr_enabled" => 1, "ost_token_ttl" => array('$gte' => $now)));
			$rs['token'] 		= $rs['ost_token'];
			$rs['token_secret']	= $rs['ost_token_secret'];
			$rs['user_id']		= $rs['ost_usa_id_ref'];
			$rs['consumer_key'] = $rs['osr_consumer_key'];
			$rs['consumer_secret'] = $rs['osr_consumer_secret'];
		}
		
		if (empty($rs)) throw new OAuthException2('The consumer_key "'.$consumer_key.'" token "'.$token.'" combination does not exist or is not enabled.');
		return $rs;
	}
	...
	public function getSecretsForSignature ($uri,$user_id,$name = ''){
		...
		$secrets = $this->db->oauth_server_registry->findOne(array(
		"osr_consumer_key" => $consumer_key,
		"ost_token_type" => $token_type, 
		"oauth_consumer_token"["ost_token"] => $token, 
		"osr_enabled" => 1, 
		"ost_token_ttl" => array('$gte' => $now)
		));
		...
		$secrets['token'] 			= $secrets['ost_token'];
		$secrets['token_secret']	= $secrets['ost_token_secret'];
		$secrets['signature_methods']		= $secrets['ocr_signature_methods'];
		$secrets['consumer_key'] 	= $secrets['osr_consumer_key'];
		$secrets['consumer_secret'] = $secrets['osr_consumer_secret'];
		if (empty($secrets))
		{
